...
export function useDropRequests(userId: string | undefined) {
  const [activeRequests, setActiveRequests] = useState<DropRequest[]>([]);
  ...
  const fetchUserRequests = async () => {
    ...
    try {
      ...
      setActiveRequests(data as DropRequest[] || []);
    } catch (error: any) {
      console.error("Error fetching user drop requests:", error);
      toast.error("Failed to load your drop requests");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDeleteRequest = async (id: string) => {
    try {
      ...
      toast.success("Drop request canceled successfully!");
      refreshRequests();
    } catch (error: any) {
